...
export const useUserStore = defineStore("user", {
  ...
  actions: {
    hidePageOverflow(val: any, id: string) {
      if (val) {
        document.body.style.overflow = "hidden";
        if (id) {
          (document.getElementById(id) as any).style.overflow = "hidden";
        }
        return;
      }
      document.body.style.overflow = "visible";
      if (id) {
        (document.getElementById(id) as any).style.overflow = "visible";
      }
    },
    ...
    async updateUserImage(data: any) {
      await $axios.post("/api/user-image", data);
    },

    async updateLinkImage(data: any) {
    ...
    getUserTheme() {
      this.$state.colors?.forEach((color: any) => {
    ...
    async updateTheme(themeId: string | number) {
    ...
    async updateLink(id: string | number, name: string, url: string) {
